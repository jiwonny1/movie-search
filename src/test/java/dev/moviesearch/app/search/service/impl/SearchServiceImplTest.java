...
//	@InjectMocks // Mapper 없이 테스트
	...
	static void setMapper() {
		...
		for(int i = 1; i <= 100000; i++) {
			allData.add(MovieDto.builder()
								...
								.releaseDate( new GregorianCalendar(2023, (i % 12), 11).getTime() )
	...
	void testSearchUsingJava() {
		// 검색 조건 설정
		List<String> keywords = List.of(new String[] {"액션"}); // 검색 키워드들
		List<Integer> genreIds = List.of(new Integer[] {18}); // 검색 장르들 (장르 아이디)
		int page = 1; // 페이지
		String orderBy = "PD"; // 정렬기준 
...
//			given(searchMapper.selectKeywords()).willReturn(keywordList);
		...
		result.forEach( movie -> System.out.println("제목: " + movie.getTitle() 
													+ " / 인지도:  " + movie.getPopularity()
													+ " / 투표수:  " + movie.getVoteCount()
													+ " / 평점:  " + movie.getVoteAverage()
													+ " / 개봉일:  " + new SimpleDateFormat("YYYY-MM-dd").format(movie.getReleaseDate())
													+ " / 장르:  " + movie.getGenreIds()
													));
		System.out.println(">> reuslt.size() : " + result.size());
	}
